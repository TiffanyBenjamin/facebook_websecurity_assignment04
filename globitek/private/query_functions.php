<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    } elseif (!has_length($user['password'], array('min' => 12, 'max' => 12))) {
      $errors[] = "Password cannot be less than 12 characters.";
    ...
    if (is_blank($user['confirm_password'])) {
      ...
      $errors[] = "Password does not match confirmed password";
    }
  ...
  function insert_user($user) {
    ...
    $hashed_password = password_hash($user['password'], PASSWORD_BCRYPT, $options);
  ...
  function validate_failed_login($failed_login, $errors=array()) {
    if (is_blank($failed_login['username'])) {
      $errors[] = "Username cannot be blank.";
    } elseif (!has_length($failed_login['username'], array('max' => 255))) {
      $errors[] = "Username must be less than 255 characters.";
    } elseif (!has_valid_username_format($failed_login['username'])) {
      $errors[] = "Username can only contain letters, numbers, and underscores.";
    }
    return $errors;
  }
  function insert_failed_login($failed_login) {
    ...
    $errors = validate_failed_login($failed_login);
    if (!empty($errors)) {
      return $errors;
    }
  ...
  function reset_failed_login($failed_login) {
    $username = '';
